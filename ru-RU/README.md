...
Когда функция-конструктор вызвана с ключевым словом `new`, она создаёт объект и устанавливает ключевое слово `this` так, чтобы оно отсылалось к этому объекту. По умолчанию, если функция-конструктор явно ничего не возвращает, она вернёт только созданные объект.

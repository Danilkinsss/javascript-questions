...
Внутри функции мы сперва определяем переменную `name` с помощью ключевого слова `var`. Это означает, что переменная будет поднята (область памяти под переменную будет выделена во время фазы создания) со значением `undefined` по умолчанию, до тех пор пока исполнение кода не дойдёт до строчки, где определяется переменная. Мы ещё не определили значение `name` когда пытаемся вывести её в консоль, поэтому выведено будет `undefined`.

Переменные, определенные с помощью `let` (и `const`), также поднимаются, но в отличие от `var`, не <i>инициализируются</i>. Доступ к ним невозможен до тех пор, пока не выполнится строка их определения (инициализации). Это называется "временная мертвая зона". Когда мы пытаемся обратиться к переменным до того момента как они определены, JavaScript выводит исключение `ReferenceError`.
...
Из-за того, что в области видимости стрелочной функции нет переменной `radius`, `this.radius` выводит `undefined`, который в свою очередь при умножении на `2 * Math.PI`, приводит к `NaN`.
...
###### 5. Что из перечисленного является НЕдопустимым?
...
В JavaScript все ключи объекта являются строками (кроме Symbol). И хотя мы можем не _выводить_ их как строки, под капотом они всегда преобразовываются к строкам.

JavaScript интерпретирует (или распаковывает) выражения. При использовании квадратных скобок JS замечает `[` и продолжает пока не встретит `]`. Только после этого он вычислит то, что находится внутри скобок.

`mouse[bird.size]`: Сперва определяется `bird.size`, которое равно `"small"`. `mouse["small"]` возвращает `true`.

Но с записью через точку так не происходит. У `mouse` нет ключа `bird`. Таким образом, `mouse.bird` равно `undefined`. Затем мы запрашиваем ключ `size`, используя точечную нотацию: `mouse.bird.size`. Так как `mouse.bird` это `undefined`, мы запрашиваем `undefined.size`. Это не является допустимым, и мы получаем ошибку на подобии `Cannot read property "size" of undefined`.
...
В JavaScript все объекты являются _ссылочными_ типами данных(reference).

Сперва переменная `c` указывает на объект. Затем мы присваеваем переменной `d` ссылку на тот же объект, что и `c`.
...
`new Number()` это встроенный конструктор функции. И хотя он выглядит как число, это не настоящее число: это объект, и у него есть ряд дополнительных особенностей.

Когда мы используем оператор `==`(оператор равенства), он лишь проверяет равенство _значений_. Оба значения равны `3`, поэтому возвращается `true`.

Однако, при использовании оператора `===`(оператор строгово равенства), мы проверяем равенство _и_ значения, _и_ типа. И в нашем случае это не так: `new Number()` это не число, это **объект**. Оба возвращают `false`.
...
###### 8. Каким будет результат выполнения кода?
...
###### 9. Каким будет результат выполнения кода?
...
4. `globalThis.greetign` во всех средах(ударение на е).
...
В JavaScript это возможно, т.к. функции это объекты! (Всё кроме примитивов является объектом).

Функция — это специальный тип объекта, который можно вызвать. Кроме того, функция — это объект со свойствами, которые мы можем вызвать.
...
JavaScript это **динамически типизированный язык**: мы не указываем, к какому типу относятся определенные переменные. Переменные могут автоматически быть преобразованы из одного типа в другой без нашего участия, что называется _неявным приведением типов_. **Приведение** это преобразование из одного типа в другой.

В этом примере JavaScript сконвертировал число `1` в строку, чтобы операция внутри функции имела смысл и вернула значение. Во время сложения числа (`1`) и строки (`'2'`) число преобразовывается к строке. Мы можем сцепить строки как например `"Hello" + "World"`. Таким образом, `"1" + "2"` возвращает `"12"`.
...
2. Увеличивает значение на 1 (инкрементирует) (теперь число равно `1`)

**Префиксный** унарный оператор `++`:

1. Увеличивает значение на 1 (инкрементирует) (число теперь равно `2`)
...
Используя `"use strict"`, можно быть уверенными, что мы по ошибке не создадим глобальные переменные. Мы ранее нигде не создавали переменную `age`, поэтому с использованием `"use strict"` возникнет `ReferenceError`. Без использования `"use strict"` ошибки не возникнет, а переменная `age` добавится в глобальный объект.
...
При использовании же `localStorage`, данные сохраняются навсегда. Очистить их можно, например, используя `localStorage.clear()`.
...
Но такой трюк нельзя проделать с `let` или `const`, т.к. у них блочная область видимости и значения для них не могут быть присвоены более одного раза.
...
###### 26. Глобальный контекст исполнения создает две вещи: глобальный объект и ключевое слово `this`
...
`String` это встроенный конструктор, к которому можно добавлять свойства. Я добавила метод к его прототипу. Строки-примитивы автоматически конвертируются к строкам-объектам, созданным функцией прототипа строки(вау). Поэтому все строки (строковые объекты) имеют доступ к этому методу!
...
###### 29. Что будет выведено на экран?
...
Тем не менее, когда мы конвертируем объект в строку, он становится `"[object Object]"`. Таким образом, мы говорим, что `a["object Object"] = 123`. Потом мы делаем то же самое. `c` это другой объект, который мы неявно конвертируем в строку. Поэтому `a["object Object"] = 456`.

Затем, когда мы выводим `a[b]`, мы имеем в виду `a["object Object"]`. Мы только что установили туда значение `456`, поэтому в результате мы и получаем `456`.
...
Это происходит из-за того, что в браузерах у нас есть не только рантайм движок, но и `WebAPI`. `WebAPI` предоставляет нам функцию `setTimeout` и много других возможностей, как например DOM.
...
###### 32. Что будет выведено на экран после клика по параграфу?
...
После клика по `p` будет выведено `p` и `div`. В цикле жизни события есть три фазы: захват, цель и всплытие. По умолчанию обработчики событий выполняются на фазе всплытия (если не установлен параметр `useCapture` на `true`). Всплытие идет с самого глубокого элемента вверх.
...
###### 33. Что будет выведено на экран?
...
В обоих случаях мы можем передать объект, на который хотим чтобы указывал `this`. Однако `.call` _выполняется сразу же_!

`.bind` возвращает _копию_ функции, но с привязанным контекстом. Он не выполняется незамедлительно.
...
К вашему сведению: в JS 8 встроенных типов: `null`, `undefined`, `boolean`, `number`, `string`, `object`, `symbol` и `bigint`. `"function"` не является отдельным типом, т.к. функции являются объектами типа `"object"`. К тому же `typeof null` возвращает `"object"`
...
###### 36. Что будет выведено на экран?
...
###### 37. Что будет выведено на экран?
...
Когда в массив добавляется значение, которое выходит за пределы длины массива, JavaScript создает так называемые "пустые ячейки". На самом деле они имеют значения `undefined`, но в консоли выводятся примерно вот так:
...
###### 38. Что будет выведено на экран?
...
Блок `catch` получает аргумент `x`. Это не тот же `x`, который определен в качестве переменной перед конструкцией `try...catch`, этот `x` вляется внутриблоковым.

Затем мы присваиваем этому аргументу значение `1` и устанавливаем значение для переменной `y`. Потом выводим в консоль значение этого внутриблокового аргумента `x`, которое равно `1`.

За пределами блока `catch` переменная `x` все еще `undefined`, а `y` равна `2`. Когда мы вызываем `console.log(x)` за пределами блока `catch`, этот вызов возвращает `undefined`, а `y` возвращает `2`.
...
Отличием примитива от объекта является то, что примитивы не имеют свойств или методов. Тем не менее, вы могли заметить, что `'foo'.toUpperCase()` преобразуется в `'FOO'` и не вызывает `TypeError`. Это происходит потому, что при попытке получения свойства или метода у примитива (например, строки), JavaScript неявно _обернет_ примитив объектом, используя один из классов-оберток (например, `String`), а затем сразу же уничтожит обертку после вычисления выражения. Все примитивы кроме `null` и `undefined` ведут себя таким образом.
...
`[1, 2]` - это начальное значение, с которым инициализируется переменная `acc`. После первого прохода `acc` будет равно `[1,2]`, а `cur` будет `[0,1]`. После конкатенации(сцепления) результат будет `[1, 2, 0, 1]`.

Затем `acc` равно `[1, 2, 0, 1]`, а `cur` равно `[2, 3]`. Мы сцепляем их и получаем `[1, 2, 0, 1, 2, 3]`.
...
Обычные функции не могут быть остановлены на полпути после вызова. Однако функцию генератор можно "остановить" на полпути, а затем продолжить с того места, где она остановилась. Каждый раз, когда в функции-генераторе встречает ключевое слово `yield`, функция возвращает значение, указанное после него. Однако обратите внимание, что функция генератора в этом случае не _return_ значение, оно _yields_ значение.

Сначала мы инициализируем функцию генератор с `i`, равным `10`. Мы вызываем функцию генератор, используя метод `next()`. Когда мы в первый раз вызываем функцию генератора, `i` равно `10`. Встречается первое ключевое слово `yield`, и мы получаем значение `i`. Генератор теперь "приостановлен", и `10` выводится в консоль.

Затем мы снова вызываем функцию с помощью метода `next()`. Она запускается с того места, где остановилась ранее, все еще с `i` равного `10`. Теперь встречается следующее ключевое слово `yield` и уже возвращается `i * 2`. `i` равно `10`, поэтому возвращается `10 * 2`, то есть `20`. Таким образом результат `10, 20`.
...
`7 + '5'` приводит к `"75"` из-за приведения(coercion). JavaScript преобразует число `7` в строку, см. вопрос 15. Мы можем объединить две строки, используя оператор `+`. `"7" + "5"` приводит к `"75"`.
...
Только первое число в строке возвращается. На основе _системы счисления_ (второй аргумент указывает, к какому типу чисел мы хотим его привести: с основание 10, в шестнадцатеричное, восьмеричное, двоичное и т.д.), `ParseInt` проверяет, являются ли символы в строке допустимыми. Как только он встречает символ, который не является допустимым числом в основании, он прекращает синтаксический анализ и игнорирует следующие символы.

`*` не является допустимым числом. Только `"7"` преобразуется в десятичную `7`. `num` теперь содержит значение `7`.
...
Аргументы передаются _значением_, если же их значение является объектом, то они передаются _ссылкой_. `birthYear` передается по значению, поскольку это строка, а не объект. Когда мы передаем аргументы по значению, создается _копия_ этого значения (см. вопрос 46).
...
С помощью оператора `throw` мы можем создавать собственные ошибки(custom errors). С этим оператором вы можете генерировать исключения. Исключением может быть <b>строка</b>, <b>число</b>, <b>логическое значение</b> или <b>объект</b>. В этом случае нашим исключением является строка `'Hello world'`.

С помощью оператора `catch` мы можем указать, что делать, если в блоке `try` выдается исключение. В нашем случае, выдается исключение: строка `'Hello world'`. `e` теперь равно той строке, которую мы записываем. Это приводит к `'Oh no an error: Hello world'`.
...
Когда функция-конструктор вызвана с ключевым словом `new`, она создаёт объект и устанавливает ключевое слово `this` так, чтобы оно отсылалось к этому объекту. По умолчанию, если функция-конструктор явным образом ничего не возвращает, она вернёт только что созданный объект.

В данном примере, функция-конструктор `Car` четко возвращает новый объект с `make` установленным на `"Maserati"`, который переписывает поведение по умолчанию. Таким образом, когда вызывается `new Car()`, _возвращенный_ объект присваевается `myCar`, приводя к результату `"Maserati"`, когда мы обращаемся к `myCar.make`.
...
Однако мы создали глобальную переменную `y`, установив `y` равную `10`. Это значение доступно в любом месте нашего кода. `y` определена и содержит значение типа `"number"`. `console.log(typeof y)` возвращает `"number"`.
...
Мы можем удалить свойства из объектов, используя ключевое слово `delete`, а также в прототипе. Удаляя свойство в прототипе, оно больше не доступно в цепочке прототипов. В этом случае функция `bark` больше не доступна в прототипе после `delete Dog.prototype.bark`, хотя мы пытаемся получить к ней доступ.
...
###### 56. Что будет выведено на экран?
...
Объект `Set` является коллекцией _уникальных_ значений: значение может появиться только один раз в наборе.
...
###### 57. Что будет выведено на экран?
...
###### 58. Что будет выведено на экран?
...
###### 59. Что будет выведено на экран?
...
Это означает, что значение `y` равно первому значению в массиве, которым является число `1`. Когда мы выводим `y`, возвращается `1`.
...
###### 60. Что будет выведено на экран?
...
Объекты можно комбинировать с помощью оператора распространения `...`. Это позволяет создавать копии пар ключ/значение одного объекта и добавлять их в другой объект. В данном случае мы создаем копии объекта `user` и добавляем их в объект `admin`. Теперь объект `admin` содержит скопированные пары ключ/значение, что приводит к `{admin: true, name: "Lydia", age: 21}`.
...
###### 61. Что будет выведено на экран?
...
С помощью метода `defineProperty` мы можем добавлять новые свойства к объекту или изменять существующие. Когда мы добавляем свойство к объекту с помощью метода `defineProperty`, они по умолчанию _не перечисляемые_. Метод `Object.keys` возвращает все имена _перечисляемых_ свойств объекта, в данном случае только `"name"`.
...
###### 62. Что будет выведено на экран?
...
Второй аргумент `JSON.stringify` - это _заменитель_(replacer). Заменитель может быть либо функцией, либо массивом, он позволяет нам контролировать, что и как должно быть преобразовано в значения.

Если заменитель является _массивом_, только свойства, имена которых включены в массив, будут добавлены в строку JSON. В данном случае, только свойства с именами `"level"` и `"health"` включены, а `"username"` исключён. `data` теперь равен `"{"level":19, "health":90}"`.

Если заменитель является _функцией_, эта функция вызывается для каждого свойства объекта, который вы преобразуете. Значение, возвращаемое из этой функции, будет значением свойства при добавлении в строку JSON. Если значение равно undefined, это свойство исключается из строки JSON.
...
###### 63. Что будет выведено на экран?
...
Унарный оператор `++` _сначала возвращает_ значение операнда, _затем увеличивает_ значение операнда. Значение `num1` равно `10`, так как функция увеличений вначале возвращает значение `num`, которое равно `10`, и только затем увеличивает значение `num`.

`num2` - это `10`, так как мы передали `num1` в `increasePassedNumber`. `number` равно `10` (значение `num1`). Опять же, унарный оператор `++` _сначала возвращает_ значение операнда, _затем увеличивает_ значение операнда. Значение `number` равно `10`, поэтому `num2` равно `10`.
...
###### 64. Что будет выведено на экран?
...
Аргумент по умолчанию реализуется в момент _вызова_(call time)! Каждый раз, когда мы вызываем функцию, создается _new_ object. Мы вызываем функцию `multiply` первые два раза, не передавая значение: `x` имеет значение по умолчанию `{number: 10}`. Затем мы записываем умноженное значение этого числа, которое равно `20`.
...
В четвертый раз мы снова передаем объект `value`. `x.number` ранее был изменен на `20`, поэтому для `x.number * = 2` выводится `40`.
...
###### 65. Что будет выведено на экран?
...
Первым аргументом, который получает метод `reduce`, является _аккумулятором_(накопителем), в данном случае `x`. Второй аргумент - это _текущее значение_, `y`. С помощью метода `reduce` мы выполняем функцию обратного вызова(callback) для каждого элемента в массиве, что в конечном итоге может привести к единственному значению.

В этом примере мы не возвращаем никаких значений, мы просто выводим значения аккумулятора и текущее значение.
...
При первом вызове аккумулятор (`x`) равен `1`, а текущее значение (`y`) равно `2`. Мы не возвращаемся из функции обратного вызова, мы выводим аккумулятор и текущее значение: `1` и `2` выводятся.

Если вы не возвращаете значение из функции, она возвращает значение `undefined`. При следующем вызове аккумулятор равен `undefined`, а текущее значение равно 3. `undefined` и `3` будут выведены.

При четвертом вызове мы снова не возвращаемся из функции обратного вызова. Аккумулятор снова равен `undefined`, а текущее значение равно `4`. `undefined` и` 4` будут выведены на экран.
...
В производном(наследуемом) классе вы не можете получить доступ к ключевому слову `this` до вызова `super`. Если вы попытаетесь это сделать, он выдаст ReferenceError: 1 и 4 приведут к ошибке обращения.

С помощью ключевого слова `super` мы вызываем конструктор родительского класса с заданными аргументами. Конструктор родителя получает аргумент `name`, поэтому нам нужно передать `name` в `super`.
...
###### 67. Что будет выведено на экран?
...
С ключевым словом `import` все импортируемые модули являются _pre-parsed_(разобраны/проанализированы заранее). Это означает, что импортированные модули запускаются _первыми_, а код в файле, который импортирует модуль, исполняется _после_.

В этом разница между `require()` в CommonJS и `import`! С помощью `require()` вы можете загружать зависимости по требованию во время выполнения кода. Если бы мы использовали `require` вместо `import`, в консоль были бы выведены `running index.js`, `running sum.js`, `3`.
...
###### 68. Что будет выведено на экран?
...
###### 69. Что будет выведено на экран?
...
Если аргумент, переданный методу `padStart`, меньше длины строки, то отступ не будет добавлен.
...
###### 70. Что будет выведено на экран?
...
###### 71. Как мы можем вывести на экран значения, которые закомментированы после оператора console.log?
...
Каждая строка выполняется до тех пор, пока не найдет первое ключевое слово `yield`. В первой строке функции есть ключевое слово `yield`: выполнение останавливается с первым yield! _Это означает, что переменная `answer` еще не определена!_

Когда мы вызываем `game.next("Yes").value`, предыдущий `yield` заменяется значением параметров, переданных функции `next()`, в данном случае `"Yes"`. Значение переменной `answer` теперь равно `"Yes"`. Условие if возвращает `false`, а `JavaScript loves you back ❤️`, выводится на экран.
...
`String.raw` возвращает строку, в которой _экранируемые_(избегающие вывода) символы (`\n`, `\v`, `\t` и т.д.) игнорируются! Обратная косая черта может быть проблемой, так как если вы можете что-то вроде:

`` const path = `C:\Documents\Projects\table.html` ``

Выполнение приведет к:

`"C:DocumentsProjects able.html"`

С `String.raw` управляющий знак будет просто проигнорирован и напечатается:

`C:\Documents\Projects\table.html`

В данном же случае, выводится строка `Hello\nworld`.
...
###### 73. Что будет выведено на экран?
...
Асинхронная функция всегда возвращает обещание(promise). `await` должен ждать пока обещание не будет разрешено(выполнено): ожидаемое обещание возвращается, когда мы вызываем `getData()`, чтобы установить `data` равным ему.
...
###### 74. Что будет выведено на экран?
...
Метод `push` изменяет исходный массив. Если вы хотите вернуть _массив_ из функции, а не _длину массива_, вы должны были вернуть `list` после добавления в него `item`.
...
###### 75. Что будет выведено на экран?
...
Поскольку `shape` заморожен, и поскольку значение `x` не является объектом, мы не можем изменить свойство `x`. `x` по-прежнему равно `10`, и `{x: 10, y: 20}` выводится.
...
###### 76. Что будет выведено на экран?
...
В нашем же случае `firstName` не существует как переменной, поэтому попытка доступа к ее значению вызовет `ReferenceError`.
...
Всякий раз, когда Javascript не может найти переменную в _текущей области видимости_, он/она поднимается вверх по [цепочке областей видимости](https://developer.mozilla.org/ru/docs/Web/JavaScript/Closures#лексическая_область_видимости) и ищет её на каждом уровне, и если достигает области верхнего уровня, также известной как **Глобальная область**, и все еще не находит нужной ссылки, то выдает `ReferenceError`.
...
###### 77. Это чистая функция(функция без побочного эффекта)?
...
###### 78. Что будет выведено на экран?
...
Если мы снова вызываем функцию `addFunction` с тем же аргументом, она сначала проверяет, получило ли оно уже это значение в своем кеше. В этом случае будет возвращено значение кэша, что экономит время выполнения. Иначе же, если аргумент не был закэширован, функция вычислит значение и сохранит его после.

Мы вызываем функцию `addFunction` три раза с одним и тем же значением: при первом вызове значение функции, когда `num` равно `10`, еще не закэшировано. Условие оператора if `num in cache` возвращает `false`, и выполняется блок else: `Calculated! 20` выводится, и значение результата добавляется в объект кеша. `cache` теперь выглядит как `{10: 20}`.

Во второй раз объект `cache` содержит значение, возвращаемое для `10`. Условие оператора if `num in cache` возвращает `true`, а `'From cache! 20'` выводится.

В третий раз мы передаем `5 * 2` в функцию, которая оценивается как `10`. Объект `cache` содержит значение, возвращаемое для `10`. Условие оператора if `num in cache` возвращает `true`, а `'From cache! 20'` выводится.
...
###### 79. Что будет выведено на экран?
...
С помощью цикла _for-in_ мы можем перебирать **перечисляемые** свойства. В массиве перечисляемые свойства являются "ключами" элементов массива, которые фактически являются их индексами. Вы можете рассматривать массив как:

`{0: "☕", 1: "💻", 2: "🍷", 3: "🍫"}`

Где ключи - перечисляемые свойства. `0` `1` `2` `3` выводятся на экран.

С помощью цикла _for-of_ мы можем выполнять итерацию **итерируемых**. Массив является итеративным. Когда мы выполняем итерацию по массиву, переменная "item" равна элементу, по которому она итерируется в данный момент, и `"☕"` `"💻"` `"🍷"` `"🍫"` выводятся.
...
###### 80. Что будет выведено на экран?
...
Элементы массива могут содержать любые значения. Числа, строки, объекты, другие массивы, null, логические значения, undefined и другие выражения, такие как даты, функции и вычисления.
...
По умолчанию аргументы имеют значение `undefined`, если только значение не было передано функции. В этом случае мы не передали значение для аргумента `name`. `name` равно `undefined`, это и будет выведено.

В ES6 мы можем перезаписать значение по умолчанию `undefined` параметрами по умолчанию. Например:

`function sayHi(name = "Lydia") { ... }`

В этом случае, если бы мы не передали никакое значение или если бы мы передали `undefined`, `name` все равно было бы равно строке `Lydia`.
...
###### 82. Что будет выведено на экран?
...
Значение ключевого слова `this` зависит от того, где вы его используете. В **методах**, как например в `getStatus`, ключевое слово `this` ссылается на _объект_, которому принадлежит метод. Метод принадлежит объекту `data`, поэтому `this` относится к объекту `data`. Когда мы пытаемся вывести `this.status`, выводится свойство `status` объекта `data`, которым является `"🥑"`.

С помощью метода `call` мы можем изменить объект, на который ссылается ключевое слово `this`. В **функциях** ключевое слово `this` относится к _объекту, которому принадлежит функция_. Мы объявили функцию `setTimeout` для объекта _global_, поэтому в функции `setTimeout` ключевое слово `this` ссылается на объект _global_. В глобальном объекте есть переменная с именем _status_ со значением `"😎"`. Когда мы выводим `this.status`, выводится `"😎"`.
...
Затем мы устанавливаем `city` равным строке `"Amsterdam"`. Это не меняет объект `person` - ссылки на этот объект не установлено.

При выводе на экран объекта `person`, возвращается неизмененный объект.
...
###### 84. Что будет выведено на экран?
...
Переменные с ключевыми словами `const` и `let` имеют _блочную видимость_. Блок - это все, что между фигурными скобками (`{}`). В этом случае в фигурных скобках операторов if/else. Вы не можете ссылаться на переменную за пределами блока, в котором она объявлена, поэтому мы получаем ReferenceError.
...
###### 85. Какая информация будет выведена на экран?
...
С помощью `!!name` мы определяем, является ли значение `name` истинным или ложным. Если имя истинное, что мы и хотим проверить, то `!name` возвращает `false`. А `!false` (это то, чем на самом деле является `!!name`) возвращает `true`.
...
###### 87. Что будет выведено на экран?
...
Чтобы получить символ по определенному индексу в строке, вы можете использовать скобочную нотацию. Первый символ в строке имеет индекс 0 и т.д. В этом случае мы хотим получить элемент с индексом 0, символ `'I'`, который и выводится на экран.
...
###### 88. Что будет выведено на экран?
...
Если вы попытаетесь установить значение параметра по умолчанию равным параметру, который определен _после_ (справа), то  это приведет к ошибке, ведь значение параметра еще не было инициализировано.
...
###### 89. Что будет выведено на экран?
...
###### 90. Что будет выведено на экран?
...
Классы являются синтаксическим сахаром(_syntactic sugar_) для конструкторов функций. Эквивалентом класса `Person` в качестве конструктора функции будет:
...
Первым подмассивом является `[ "name", "Lydia" ]`, где `x` равно `"name"`, и `y` равно `"Lydia"`, которые и выводятся.
Вторым подмассивом является `[ "age", 21 ]`, где `x` равно `"age"`, и `y` равно `21`, которые и выводятся.
...
Приведенный выше пример сработает. В нём будет возвращён массив `[ 'banana', 'apple', 'orange', 'pear' ]`
...
###### 96. Что будет выведено на экран?
...
###### 97. Что будет выведено на экран?
...
`Symbol` не является _перечисляемый_. Метод `Object.keys` возвращает все _перечисляемые_ свойства ключа для объекта. `Symbol` не просматривается таким образом, и возвращается пустой массив. При попытке вывести весь объект, будут видны все свойства, даже неперечисляемые.

Это одно из многих качеств символа: помимо представления совершенно уникального значения (которое предотвращает случайное конфликт/коллизию имен в объектах, например, при работе с 2 библиотеками, которые хотят добавить свойства к одному и тому же объекту), вы также можете "скрыть" свойства объектов таким образом (хотя и не полностью. Вы можете получить доступ к символам, используя метод `Object.getOwnPropertySymbols()`).
...
###### 98. Что будет выведено на экран?
...
С помощью параметра "и остальное/и тд" `...y` мы помещаем все оставшиеся аргументы в массив. В данном случае, такими аргументами являются `2`, `3` и `4`. Значение `y` является массивом, содержащим все остальные параметры. В этом случае значение `x` равно `1`, поэтому при выводе на экран мы видим `[x, y]`, `[1, [2, 3, 4]]`.

Функция `getUser` получает объект. В стрелочных функциях нам _не нужно_ писать фигурные скобки, если мы просто возвращаем одно значение. Однако, если вы хотите мгновенно вернуть _object_ из стрелочной функции, вы должны написать его между круглыми скобками, иначе все, что находится между двумя фигурными скобками, будет интерпретироваться как оператор блока. В данном случае, код между фигурными скобками не является допустимым кодом JavaScript, поэтому выдаётся `SyntaxError`.

Следующая же функция вернула бы объект:
...
_Ошибка типа_(TypeError) выдаётся, когда значение не соответствует ожидаемому типу. JavaScript ожидал, что `name` будет функцией, так как мы пытаемся вызвать ее. Однако это была строка, поэтому выдается ошибка "TypeError": `name` не является функцией!

_Синтаксическая ошибка_(SyntaxError) генерируются, когда вы написали что-то, что не является допустимым в JavaScript, например, когда вы написали слово `return` как `retrun`.
_Ошибка ссылки_(ReferenceErrors) генерируется, когда JavaScript не может найти ссылку на значение, к которому вы пытаетесь получить доступ.
...
###### 100. Какое значение будет выведено на экран?
...
###### 101. Какое значение будет выведено на экран?
...
`(null || false || "")`: все операнды являются ложными значениями. Это означает, что последний операнд `""` возвращается. `two` содержит `""`.

`([] || 0 || "")`: пустой массив `[]` является истинным значением. Это первое истинное значение, которое возвращается. `three` содержит `[]`.
...
###### 102. Какое значение будет выведено на экран?
...
Используя обещания мы по сути своей говорим: _"Я хочу выполнить эту функцию и откладываю ее, пока она выполняется, поскольку это может занять некоторое время. Только когда определенное значение разрешено (или отклонено), и когда стек вызовов пуст, я хочу использовать это значение_".

Мы можем получить это значение с помощью как ключевого слова `.then`, так и `await` в функции `async`. И хотя мы можем получить значение обещания с помощью и `.then`, и `await`, они работают немного по-разному.

В `firstFunction` мы (вроде) отложили функцию `myPromise` во время ее работы, но продолжили выполнение другого кода, который в данном случае `console.log ('second')`. Затем функция разрешается строкой `I have resolved`, которая затем выводится на экран после того, как она увидела, что стек вызовов пуст.

Используя ключевое слово `await` в `secondFunction`, мы буквально приостанавливаем выполнение асинхронной функции до тех пор, пока значение не будет разрешено прежде чем переходить к следующей строке кода.

Это означает, что мы ожидали разрешения `myPromise` со значением `I have resolved`, и только когда это произошло, мы перешли к следующей строке: `second` была выведена на экран последней.
...
###### 103. Какое значение будет выведено на экран?
...
Первым значением является `1`, которое является числовым. `1 + 2` возвращает число `3`.

Однако же второе значение это `"Lydia"`. `"Lydia"` является строкой, а `2` является числом: `2` приводится к строке. `"Lydia"` и `"2"` объединяются, что приводит к строке `"Lydia2"`.

`{name: "Lydia"}` является объектом. Ни число, ни объект не являются строкой, поэтому они оба приводятся к строке. Всякий раз, когда мы приводим обычный объект, он становится `"[object Object]"`. `"[object Object]"`, объединенный с `"2"`, становится `"[object Object]2"`.
...
В JavaScript у нас есть два способа доступа к свойствам объекта: обозначение в скобках или обозначение в точках. В этом примере мы используем точечное обозначение (`colorConfig.colors`) вместо скобочного (`colorConfig["colors"]`).

В точечном обозначении JavaScript пытается найти свойство объекта с точно таким именем. В этом примере JavaScript пытается найти свойство с именем `colors` в объекте `colorConfig`. Свойства с именем `colors` в нём не существует, поэтому возвращается `undefined`. Затем мы пытаемся получить доступ к значению первого элемента, используя `[1]`. Мы не можем сделать этого со значением `undefined`, поэтому выдаётся `TypeError`: `Cannot read property '1' of undefined`.

JavaScript интерпретирует (или распаковывает) операторы. Когда мы используем скобочные обозначения, он видит первую открывающую скобку `[` и продолжает работать, пока не найдет закрывающую скобку `]`. Только полсе этого он оценит выражение. Если бы мы использовали `colorConfig[colors [1]]`, он бы вернул значение свойства `red` объекта `colorConfig`.
...
Под капотом смайлики - это юникоды(unicode). Юникод для смайлика сердца это `"U+2764 U+FE0F"`. Они всегда одинаковы для одного и того же смайлика, поэтому мы сравниваем две одинаковые строки друг с другом, что и возвращает `true`.
...
###### 109. Каким будет конечный результат?
...
Мы устанавливаем значение свойства `favourFood` для объекта `info` равным строке со смайликом пиццы, `'🍕'`. Строка является примитивным типом данных. В JavaScript примитивные типы данных не взаимодействуют по ссылкам.

В JavaScript примитивные типы данных (все, что не являются объектом) передаются как _значение_. В этом случае мы устанавливаем значение свойства `favourFood` объекта `info` равным значению первого элемента в массиве `food`, в данном случае это строка с emoji пиццы (`'🍕'`). Строка является примитивным типом данных и взаимодействует по значению (см. мой [пост в блоге](https://www.theavocoder.com/complete-javascript/2018/12/21/by-value-vs-by-reference), если вы заинтересованы в получении дополнительной информации).

Затем мы меняем значение свойства `favourFood` объекта `info`. Массив `food` не изменился, поскольку значение `favourFood` было просто _скопировано_ из значения первого элемента в массиве и не имеет ссылки на то же место в памяти, что и элемент на `food[0]`. Когда мы выводим `food`, это все равно исходный массив, `['🍕', '🍫', '🥑', '🍔']`.
...
- A: Преобразует JSON в значение JavaScript
- B: Преобразует объект JavaScript в JSON
- C: Преобразует любое значение JavaScript в JSON
- D: Преобразует JSON непосредственно в объект JavaScript
...
С помощью метода `JSON.parse()` мы можем преобровать строку JSON в значение JavaScript.
...
###### 111. Какое значение будет выведено в результате выполнения кода? 
...
Переменные с ключевым словом `let` (и `const`) поднимаются в начало функции, но в отличие от `var`, <i>не инициализируются</i>. Они недоступны до того, как мы объявим (инициализируем) их строку. Это называется "временной мертвой зоной". Когда мы пытаемся получить доступ к переменным до их объявления, JavaScript выдает `ReferenceError`.

Если бы мы не объявили переменную `name` в функции `getName`, движок javascript посмотрел бы вниз по _цепочки области действия_. Внешняя область имеет переменную с именем `name` со значением `Lydia`. В этом случае он бы вывел на экран `Lydia`.
...
###### 112. Какое значение будет выведено в результате выполнения кода?
...
###### 113. Какое значение будет выведено в результате выполнения кода?
...
###### 114. Что произойдет в результате выполнения кода?
...
При добавлении пары ключ/значение с использованием метода `set`, имя ключа будет равно значению первого аргумента, переданного в функцию `set`, а значением ключа будет второй аргумент, переданный в функцию `set`. В данном случае ключом является _функция_ `() => 'greeting'`, а значением - `'Hello world'`. `myMap` теперь это `{ () => 'greeting' => 'Hello world!' }`.
...
###### 116. Какое значение будет выведено в результате выполнения кода?
...
Функции `changeAge` и `changeAgeAndName` имеют параметр по умолчанию, а именно _только_ созданный объект `{ ...person }`. Этот объект имеет копии всех ключей/значений объекта `person`.
...
###### 117. Который из следующих вариантов вернёт значение `6`?
...
С помощью оператора распространения (spread) `...` мы можем _распределить_ итерации(итерируемые значения) в отдельньные элементы. `sumValues` принимает три аргумента:`x`, `y` и `z`. `...[1, 2, 3]` приведет к перечню `1, 2, 3`, который мы и передаём функции `sumValues`.
...
###### 118. Что будет выведено на экран?
...
С арифметическим оператором `+=` мы увеличиваем значение `num` на `1`. `num` имеет начальное значение `1`, поэтому `1 + 1` равно `2`. Элементом второго индекса в массиве `list` является 🥰, поэтому `console.log (list [2])` выводит на экран 🥰.
...
###### 119. Что будет выведено на экран?
...
С необязательным оператором связывания `?.` нам больше не нужно явно проверять, действительны ли более глубокие вложенные значения или нет. Если мы пытаемся получить доступ к свойству с (_нулевым_) значением `undefined` или `null`, выражение замыкается и возвращает `undefined`.
...
###### 120. Что будет выведено на экран?
...
Мы передали условие `groceries.indexOf("banana")` в оператор `if`. `groceries.indexOf("banana")` возвращает `0`, что является ложным значением. Поскольку условие в операторе `if` ложно, выполняется код в блоке `else`, и выводится `We don't have to buy bananas!`.
...
###### 121. Что будет выведено на экран?
...
Метод `language` является `сеттером`(setter). Сеттеры не содержат действительного значения, их целью является _изменение свойств_. При вызове метода `setter`, возвращается `undefined`.
...
###### 122. Что будет выведено на экран?
...
###### 123. Что будет выведено на экран?
...
###### 124. Что будет выведено на экран?
...
###### 125. Что будет выведено на экран?
...
###### 126. Что будет выведено на экран?
...
С помощью метода `Intl.NumberFormat` мы можем форматировать числовые значения в любой локали(locale). Мы форматируем числовое значение `130` для локали `en-US` как `unit` в `mile-per-hour`, что приводит к `130 mph`. Числовое значение `300` для локали `en-US` в качестве `currency` в `USD` приводит к `$300.00`.
...
###### 127. Что будет выведено на экран?
...
Деструктурируя объекты, мы можем распаковать значения из правого объекта и присвоить это распакованное значение значению того же по имени свойства в левом объекте. В этом случае мы присваиваем значение "💀" `spookyItems[3]`. Это означает, что мы модифицируем массив `spookyItems`, добавляем к нему "💀". При выводе `spookyItems` мы видим на экране ` ["👻", "🎃", "🕸", "💀"]`.
...
###### 128. Что будет выведено на экран?
...
###### 129. Что будет выведено на экран?
...
Переменные, объявленные с ключевым словом `const`, не имеют ссылки до их инициализации: это называется _временная мертвая зона_. В функции `getInfo` переменная `randomValue` находится в области видимости `getInfo`. В строке, где мы хотим вывести значение `typeof randomValue`, переменная `randomValue` еще не инициализирована: выдается `ReferenceError`! Движок не пошел по цепочке областей видимости, так как мы объявили переменную `randomValue` в функции `getInfo`.
...
###### 130. Что будет выведено на экран?
...
В блоке `try` мы выводим в лог ожидаемое значение переменной `myPromise`: `"Woah some cool data"`. Поскольку в блоке `try` не было выдано никаких ошибок, код в блоке `catch` не запускается. Код в блоке `finally` _всегда_ выполняется, `"Oh finally!"` также выводится на экран.
...
###### 131. Что будет выведено на экран?
...
С помощью метода `flat` мы можем создать новый плоский массив. Глубина сплюснутого массива зависит от значения, которое мы передаем. В этом случае мы передали значение `1` (которое на самом деле не нужно, т.к. это значение по умолчанию), что означает, что будут объединены только массивы первой глубины: `['🥑']` и `['✨', '✨', ['🍕', '🍕']]` в данном случае. Объединение этих двух массивов приводит к `['🥑', '✨', '✨', ['🍕', '🍕']]`.
...
###### 132. Что будет выведено на экран?
...
- B: `sum.sum(4)`
- C: `sum.default(4)`
- D: Нельзя импортировать значения по умолчанию используя `*`, можно только именованные экспорты
...
###### 135. Что будет выведено на экран?
...
Сначала мы устанавливаем новое свойство `name` для объекта Proxy (`person.name = "Lydia"`). Вызывается `set` и выводится на экран `"Added a new property!"`.

Затем мы обращаемся к значению Proxy-объекта. Вызывается свойство `get` объекта `handler`. `"Accessed a property!"` выводится.
...
###### 136. Которое из перечисленных действий может модифицировать объект `person`?
...
С помощью `Object.seal` мы предотвращаем как _добавление_ новых свойств, так и _удаление_ уже существующих.
...
###### 137. Которое из перечисленных действий может модифицировать объект `person`?
...
###### 138. Что будет выведено на экран?
...
Сначала мы вызываем `myFunc()` без передачи каких-либо аргументов. Поскольку мы не передаем аргументы, `num` и `value` получают свои значения по умолчанию: `num` равно `2`, а `value` возвращаемое значение функции `add`. В функцию `add` мы передаем в качестве аргумента `num` со значением `2`. `add` возвращает `4`, что является значением `value`.

Затем мы вызваем `myFunc(3)` и передаем значение `3` в качестве значения аргумента `num`. Мы не передаем аргумент для `value`. Поскольку мы не передаем значение для аргумента `value`, он получает значение по умолчанию: возвращаемое значение функции `add`. В `add` мы передаем `num`, значение которого равно `3`. `add` возвращает `6`, что является значением `value`.
...
В ES2020 мы можем добавлять приватные переменные в классы с помощью символа `#`. Мы не можем получить доступ к этим переменным вне класса. Когда мы пытаемся вывести на экран `counter.#number`, выдается `SyntaxError`: мы не можем получить доступ вне класса `Counter`!
...
###### 141. Что будет выведено на экран?
...
Функция `addHobby` получает два аргумента: `hobby` и `hobbies` где последний в качестве значения по умолчанию имеет массив `hobbies` в объекте `person`.

В начале, мы вызываем функцию `addHobby` и передаем `"running"` в качестве значения для `hobby`, а пустой массив в качестве значения для `hobbies`. Так как мы передаем пустой массив в качестве значения для `hobbies`, `"running"` добавляется к этому пустому массиву.
...
После добавления `dancing` и `baking`, значение `person.hobbies` стало равно `["coding", "dancing", "baking"]`
...
###### 142. Что будет выведено на экран?
...
Мы создаем переменную `pet`, которая является экземпляром класса `Flamingo`. Когда мы создаем этот экземпляр, вызывается `constructor` для `Flamingo`. Сначала на экран выводится `"I'm pink. 🌸"`, после чего мы вызываем `super()`. `super()` вызывает конструктор родительского класса `Bird`. Конструктор в `Bird` вызывается и выводится `"I'm a bird. 🦢"`.
...
###### 143. Которые из вариантов приведут к ошибке?
...
Ключевое слово `const` просто означает, что мы не можем _повторно объявить_ значение этой переменной, оно доступно только для чтения. Однако само значение не является неизменным. Свойства массива `emojis` можно изменить, например, добавив новые значения, объединив их или установив длину массива равной 0.
...
###### 144. Что нам нужно добавить к объекту `person`, чтобы получить `["Lydia Hallie", 21]` в качестве значения `[...person]`?
...
- A: Ничего, объекты итерируются по умолчанию
...
###### 145. Что будет выведено на экран?
...
Условие `if` внутри цикла `forEach` проверяет, является ли значение `num` истинным или ложным. Поскольку первое число в массиве `nums` равно `0`, то есть является ложным, блок оператора `if` не будет выполнен. `count` увеличивается только для остальных 3 чисел в массиве `nums`: `1`, `2` и `3`. Поскольку `count` увеличивается на 1 трижды, значение `count` будет равно `3`.
...
###### 146. Что будет выведено на экран?
...
`?` позволяет нам опционально получить доступ к более глубоким вложенным свойствам внутри объектов. Мы пытаемся вывести элемент с индексом `1` в подмассиве с индексом `1` массива `fruits`. Если подмассив с индексом `1` в массиве `fruits` не существует, он просто вернет `undefined`. Если подмассив с индексом `1` в массиве `fruits` существует, но в этом подмассиве нет элемента с индексом `1`, он также вернет значение `undefined`.

В первом случае, мы пытаемся вывести второй элемент `['🍍']` подмассива `[['🍊', '🍌'], ['🍍']]`. Этот подмассив содержит только один элемент, что означает, что в индексе `1` нет элемента, поэтому возвращается значение `undefined`.

Затем мы вызываем функцию `getFruits` без передачи значения в качестве аргумента, что означает, что `fruits` по умолчанию имеет значение `undefined`. Поскольку мы условно связываем элемент с индексом `1` массива `fruits`, он возвращает значение `undefined`, поскольку этого элемента с индексом `1` не существует.

В конце, мы попытаемся вывести второй элемент `['🍊', '🍌']` подмассива `['🍍'], ['🍊', '🍌']`. Элемент с индексом `1` в этом подмассиве — `🍌`, который и выводится на экран.
...
###### 147. Что будет выведено на экран?
...
###### 148. Что будет выведено на экран?
...
###### 149. Что будет выведено на экран?
...
В начале, мы вызываем метод `slice` для массива фруктов. Метод `slice` не изменяет исходный массив, а возвращает значение, которое было вырезано из массива: в данном примере, банановый смайлик (`'🍌'`).
...
В конце, мы вызываем метод `unshift` для массива `fruit`, который изменяет исходный массив, добавляя предоставленное значение, в данном случае `'🍇'`, в качестве первого элемента в массиве. Массив фруктов теперь состоит из `['🍇', '🍊', '🍎']`.
...
###### 150. Что будет выведено на экран?
...
`cat` также является объектом, и `animals[cat]` на самом деле означает, что мы перезаписываем значение `animals["object Object"]` новыми свойствами кота.

Вызов `animals[dog]`, или фактически `animals["object Object"]`(поскольку преобразование объекта `dog` в строку приводит к `"object Object"`), возвращает `{ emoji: "🐈", name: "Sara"}`.
...
###### 151. Что будет выведено на экран?
...
###### 152. Что будет выведено на экран?
...
Метод `Promise.all` выполняет переданные обещания(promisses) параллельно. Если одно обещание не выполняется, метод `Promise.all` _отколняется_ со значением отклоненного обещания. В этом случае `promise3` отклонен со значением `"Third"`. Мы перехватываем отклоненное значение в цепочке методов `catch` при вызове `runPromises`, чтобы перехватывать любые ошибки внутри функции `runPromises`. Выводится только`"Third"`, так как `promise3` отклонено с этим значением.
...
###### 153. Каким должно быть значение `method` для вывода на экран `{ name: "Lydia", age: 22 }`?
...
Метод `fromEntries` превращает двумерный массив в объект. Первый элемент в каждом подмассиве будет ключом, а второй элемент в каждом подмассиве будет значением. В этом случае мы проходимся по массиву `keys`, который возвращает массив, первый элемент которого является элементом массива ключей(keys array) с текущим индексом, а второй элемент является элементом массива значений(values array) с текущим индексом.

Это создает массив подмассивов, содержащий правильные ключи и значения, поэтому выводится `{ name: "Lydia", age: 22 }`
...
###### 154. Что будет выведено на экран?
...
Значением по умолчанию для `address` является пустой объект `{}`. Когда мы устанавливаем переменную `member` равной объекту, возвращаемому функцией `createMember`, мы не передаем значение для `address`, что означает, что значение `address` является пустым объектом по умолчанию `{}`. Пустой объект является истинным значением, что означает, что условие `address ? address : null` возвращает `true`. Значением `address` является пустой объект `{}`.
...
###### 155. Что будет выведено на экран?
...
Условие в операторе `if` проверяет, равно ли значение `!typeof randomValue` "строке". Оператор `!` преобразует значение в логическое значение. Если значение истинно, то возвращаемое значение будет `false`; если значение ложное, то возвращаемое значение будет `true`. В данном случае возвращаемое значение `typeof randomValue` является истинным значением `"number"`, что означает, что значение `!typeof randomValue` является логическим значением `false`.

`!typeof randomValue === "string"` всегда возвращает `false`, поскольку на самом деле мы проверяем `false === "string"`. Поскольку условие вернуло `false`, запускается блок кода оператора `else`, и на экран выводится сообщение `Yay it's a string!`.

...
###### 100. Какое значение будет выведено?
...
###### 103. Какое значение будет выведено?
...
Первым является `1`, которое является числовым значением. `1 + 2` возвращает число `3`.

Однако второе значение это `"Lydia"`. `"Lydia"` является строкой, а `2` является числом: `2` приводится к строке. `"Lydia"` и `"2"` объединяются, что приводит к строке `"Lydia2"`.
